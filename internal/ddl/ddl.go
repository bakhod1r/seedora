...
const (
	...
	MySQL    Dialect = "mysql"
	// SQLServer is the third family. It differs from all three above in the two
	// places this package cares about: identifiers are bracketed rather than
	// quoted, and a key that fills itself is IDENTITY rather than a serial type
	// or an AUTO_INCREMENT suffix.
	SQLServer Dialect = "sqlserver"

	// The rest are ANSI-quoting engines that differ only in their type names,
	// which is the whole reason this is a string and a switch: an engine whose
	// DDL is Postgres-shaped needs a constant here and a case in Types, not a
	// package of its own.
	...
	// CQL is Cassandra's dialect. It is SQL-shaped enough to render a CREATE
	// TABLE and nothing more: no foreign keys, no joins, and a primary key that
	// decides the physical layout rather than describing it.
	CQL Dialect = "cql"
	// Document is the dialect of the engines with no DDL at all — MongoDB,
	// Elasticsearch, DynamoDB, Neo4j, Redis. The schema editor renders nothing
	// for it, because there is nothing it could render that the engine would
	// accept.
	Document Dialect = "document"
)
...
func quoteAs(d Dialect, s string) string {
	...
	case SQLServer:
		...
		// which is the default for the driver but not for every tool a script
		// exported from here might be pasted into. Brackets always work.
		return "[" + strings.ReplaceAll(s, "]", "]]") + "]"
	}
...
func Types(d Dialect) []string {
	...
	case SQLServer:
		// IDENTITY rides along with the type for the same reason AUTO_INCREMENT
		// does on MySQL: it is where the engine wants it, and a key column that
		// does not fill itself is the easiest thing to get wrong here.
		return []string{
			"BIGINT IDENTITY(1,1)", "BIGINT", "INT", "NVARCHAR(255)", "NVARCHAR(50)",
			"NVARCHAR(MAX)", "BIT", "DATETIME2", "DATE", "DECIMAL(10,2)", "FLOAT",
			"UNIQUEIDENTIFIER", "VARBINARY(MAX)",
		}
	case Oracle:
		return []string{
			"NUMBER GENERATED BY DEFAULT AS IDENTITY", "NUMBER(19)", "NUMBER(10)",
			"VARCHAR2(255)", "VARCHAR2(50)", "CLOB", "NUMBER(1)", "TIMESTAMP",
			"DATE", "NUMBER(10,2)", "BINARY_DOUBLE", "RAW(16)", "BLOB",
		}
	case HANA:
		return []string{
			"BIGINT GENERATED BY DEFAULT AS IDENTITY", "BIGINT", "INTEGER",
			"NVARCHAR(255)", "NVARCHAR(50)", "NCLOB", "BOOLEAN", "TIMESTAMP",
			"DATE", "DECIMAL(10,2)", "DOUBLE", "VARBINARY(16)", "BLOB",
		}
	case Vertica:
		return []string{
			"IDENTITY", "INT", "VARCHAR(255)", "VARCHAR(50)", "LONG VARCHAR",
			"BOOLEAN", "TIMESTAMPTZ", "DATE", "NUMERIC(10,2)", "FLOAT",
			"UUID", "VARBINARY",
		}
	case Firebird:
		return []string{
			"BIGINT GENERATED BY DEFAULT AS IDENTITY", "BIGINT", "INTEGER",
			"VARCHAR(255)", "VARCHAR(50)", "BLOB SUB_TYPE TEXT", "BOOLEAN",
			"TIMESTAMP", "DATE", "DECIMAL(10,2)", "DOUBLE PRECISION", "BLOB",
		}
	case ClickHouse:
		// Nullability is part of the type here rather than a column constraint,
		// and there is no identity of any kind: ClickHouse has no sequences.
		return []string{
			"UInt64", "Int64", "Int32", "String", "FixedString(36)", "UInt8",
			"Bool", "DateTime64(3)", "Date", "Decimal(10,2)", "Float64", "UUID",
		}
	case Snowflake, Databricks:
		return []string{
			"BIGINT", "INT", "VARCHAR(255)", "VARCHAR(50)", "STRING", "BOOLEAN",
			"TIMESTAMP_NTZ", "DATE", "DECIMAL(10,2)", "DOUBLE", "BINARY",
		}
	case BigQuery:
		return []string{
			"INT64", "STRING", "BOOL", "TIMESTAMP", "DATE", "NUMERIC",
			"FLOAT64", "BYTES", "JSON",
		}
	case Trino:
		return []string{
			"BIGINT", "INTEGER", "VARCHAR(255)", "VARCHAR", "BOOLEAN",
			"TIMESTAMP(3)", "DATE", "DECIMAL(10,2)", "DOUBLE", "VARBINARY",
		}
	case CQL:
		return []string{
			"bigint", "int", "text", "boolean", "timestamp", "date",
			"decimal", "double", "uuid", "timeuuid", "blob",
		}
	case Document:
		// Nothing: these engines take no DDL, so offering a type list would be
		// offering a menu that leads nowhere.
		return nil
	default:
		return []string{
...
func DialectFor(engine string) Dialect {
	...
	case strings.EqualFold(engine, "MySQL"), strings.EqualFold(engine, "MariaDB"):
		return MySQL
	case strings.EqualFold(engine, "SQL Server"), strings.EqualFold(engine, "SQLServer"):
		return SQLServer
	case strings.EqualFold(engine, "Oracle"):
		return Oracle
	case strings.EqualFold(engine, "SAP HANA"), strings.EqualFold(engine, "HANA"):
		return HANA
	case strings.EqualFold(engine, "Vertica"):
		return Vertica
	case strings.EqualFold(engine, "Firebird"):
		return Firebird
	case strings.EqualFold(engine, "ClickHouse"):
		return ClickHouse
	case strings.EqualFold(engine, "Snowflake"):
		return Snowflake
	case strings.EqualFold(engine, "BigQuery"):
		return BigQuery
	case strings.EqualFold(engine, "Databricks"):
		return Databricks
	case strings.EqualFold(engine, "Trino"):
		return Trino
	case strings.EqualFold(engine, "Cassandra"), strings.EqualFold(engine, "ScyllaDB"):
		return CQL
	case strings.EqualFold(engine, "MongoDB"), strings.EqualFold(engine, "Elasticsearch"),
		strings.EqualFold(engine, "DynamoDB"), strings.EqualFold(engine, "Neo4j"),
		strings.EqualFold(engine, "Redis"):
		return Document
	}
	return Postgres
}
